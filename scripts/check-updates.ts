#!/usr/bin/env tsx
/**
 * Thai Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://www.ratchakitcha.soc.go.th';
const PORTAL_NAME = 'Royal Gazette (Ratchakitcha)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/thai-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Thai Law MCP — Data Freshness Check');
